...
Este potencial fue inicialmente propuesto por Wiletz \textbf{CITAS WILETZ} y continuado por Dorso \textbf{CITAS DORSO PAULI} y utilizado en múltiples trabajos desde entonces.
...
En el estudio del potencial de Pauli, resulta interesante analizar si pueden existir fases de no gaseosas de un sistema que interactua mediante este potencial.
Sin embargo, las simulaciones vía el algoritmo de Metropolis-Montecarlo tiene la inherente incapacidad de analizar observables asociados a la evolución temporal del sistema.
Esto hace que algunos indicadores como el \textit{coeficiente de Lindemann} resulten dificiles de interpretar a la hora de observar transiciones de fase.
...
Sin embargo, la función de distribución radial $g(r)$ analiza la probabilidad de encontrar 2 partículas a una distancia $r$ dada, lo cual facilmente podemos obtener de una simulación de Montecarlo.
Esta función de distribución radial se define según \textbf{CITAR HAILE}
\begin{equation}{\label{eq:def_gr}}
 g(r) = \frac{2}{N}\frac{\left< N(r,\Delta r) \right>}{\rho V(r,\Delta r)} \approx \frac{2}{N}\frac{\left< N_k(r,\Delta r) \right>}{\rho 4\pi  r^2\Delta r}
\end{equation}
donde $N(r,\Delta r)$ es la cantidad de pares de partículas cuya distancia se encuentra entre $r$ y $r+\Delta r$ y $V(r,\Delta r)$ es el volumen de un cascarón esférico de radio interior $r$ y 
radio exterior $r+\Delta r$.
La aproximación vale para $\Delta r$ pequeño, lo cual es ciertamente deseable para tener una discretización fina.

Para un sistema gaseoso, esperamos que $g(r)$ tienda a 1 rapidamente, dado que el sistema es caótico y tiende a tener partículas a todas las distancias.
...
En los 3 casos, debemos tener $g(r)\to 1$ para $r$ suficientemente grande.
...
La Materia Nuclear (NM) consiste en un sistema termodinámico de nucleones electricamente neutros, interactuando unicamente mediante interacción nuclear (sin Coulomb).
...
Existen múltiples formulaciones de este potencial, pero aquí nos concentraremos en el conocido como QCNM, cuyo potencial de interacción entre \textit{todos los nucleones} es\textbf{CITA DORSO NUCLEAR}
...
La intensidad del potencial se tomó $V_o=25.93$MeV con potencias $p_1=6.2$ y $p_2=3.0$, distancias características $r_1=1.757$fm y $r_2=1.771$fm y modulación exponencial $d=3.35$fm y $a=5/6$fm.
De esta manera, hay un mínimo de potencial en $r\approx 2.138$fm de $-5.722$MeV.


El hecho de que este potencial trate de forma idéntica protones y neutrones no es una característica general.
Existen potenciales como el CMD que separan la interacción entre $V_{np}$ y $V_{nn/pp}$ donde la interacción entre nucleones idénticos es puramente repulsiva mientras que la interacción proton-neutron 
es atractivo a distancia finita y repulsivo a distancias cortas.

En general, los parámetros de todos estos potenciales se ajustan para que la energía y la densidad de saturación coincidan con los valores experimentales $E_{NM}^{sat}=16$MeV y $\rho_{sat}=0.16$fm$^{-3}$.